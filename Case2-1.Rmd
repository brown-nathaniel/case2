...
p1<- ggplot(data = pdat, aes(x=race, fill=race)) + geom_bar() + theme_bw()
p2<- ggplot(data = pdat, aes(x=symptom, fill=symptom)) + geom_bar() + theme_bw()
...
g3 <- ggplot(data = dat, aes(x=race, y=symptom)) + geom_boxplot(aes(fill=gender))+theme_bw()
...
```{r,echo=F}
p3<-ggplot(data = dat, aes(x=race))+geom_bar(aes(fill=scan), position="dodge")+theme_bw()+labs(x="Race",y="Frequency",fill="Scanned or Not")
p4<-ggplot(data = dat, aes(x=symptom)) +geom_bar(aes(fill=scan), position="dodge") +theme_bw()+labs(x="Number of Symptoms", y="Frequency", fill="Scanned or Not")
p5<-ggplot(data = dat, aes(x=gender))+geom_bar(aes(fill=scan),position="dodge")+theme_bw()+labs(x="Gender", y="Frequency", fill="Scanned or Not")
grid.arrange(p3, p4,p5, nrow=3)
```
Lastly, we observe the number of people from each group who got neurological/CT scan and who did not. We see that the proportion of the patients who got CT scan is similar across race and gender, while the patients who showed 1 major stroke symptom are more likely to get CT scan than patients with less or more stroke symptoms.
