...
ylim <- scale_y_continuous(limits = c(0,1))
p3<-ggplot(data = dat, aes(x=race, y=racescan))+geom_col(width = 0.25, position="dodge", fill = "dark green")+theme_bw()+labs(x="Race",y="Proportion",fill="Scanned or Not", title ="Proportion of Patients Who Saw Neurologist by Group") + theme(plot.title = element_text(size=15, hjust = 0.5)) + ylim
p4<-ggplot(data = dat, aes(x=symptom, y=symptomscan)) +geom_col(width=0.5, position="dodge", fill = "red") +theme_bw()+labs(x="Number of Symptoms", y="Proportion", fill="Scanned or Not") + ylim
p5<-ggplot(data = dat, aes(x=gender, y=genderscan))+ geom_col(width=0.25,position="dodge", fill="blue")+theme_bw()+labs(x="Gender", y="Proportion", fill="Scanned or Not") + ylim
