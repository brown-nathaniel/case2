...
#   nums[x>c(nums) & x<=c(nums,Inf)]
# }
# uppertimes <- 1:ceiling(max(dat$nctdel)) #upper bounds for time categories

dat$timecat <- ceiling(dat$nctdel)
dat$timecat[dat$timecat==0] <- 1
  
datcat$time <- ceiling(datcat$nctdel)
datcat
```
