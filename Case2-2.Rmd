...
library(glmnet)
library(arm)
>>>>>>> 08f6a62cb31a05726b263f1b266f162e34b94e3c
```
...
To perform logistic regression on the data, we categorized the time to event variable into groups of **BLANK** minutes, with all events occuring after **BLANK** minutes representing one group, since the sample size was so lo after that time (only `r sum(dat$nctdel > 5)` observations). The LASSO Regularization has the best fit. (the best fit depends on bin size and idk what bin size to use).
